...
RSpec.configure do |config|
  ...
  config.before(:each) do
    ...
    allow(Iugu).to receive(:api_key).and_return('ak_test_XLoo19QDn9kg5JFGU70x12IA4NqbAv')
    allow(PaymentEngines).to receive(:configuration).and_return({})
    Sidekiq::Testing.inline!
    allow(CatarseIugu::VerifyIuguWorker).to receive(:perform_in).and_return(true)
  end
